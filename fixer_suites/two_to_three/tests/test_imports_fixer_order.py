...
def fixer_fixture(two_to_three_test_case):
    ...
    modules = mapping2.copy()

    from ..fixes.fix_imports import MAPPING as mapping1

    for key in ("dbhash", "dumbdbm", "dbm", "gdbm"):
        modules[key] = mapping1[key]

    test_case = two_to_three_test_case("", fix_list=["imports", "imports2"])
...
def test_after_local_imports_refactoring(fixer):
    for fix in ("imports", "imports2"):
        fixer.fixer = fix
        fixer.assert_runs_after("import")
