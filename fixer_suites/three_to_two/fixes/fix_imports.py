...
from_import_match = (
    "from_import=import_from< 'from' {fmt_name} 'import' ['('] imported=any [')'] >"
)
...
def all_patterns(name):
    ...
    if "." in name:
        ...
        dotted_name = dotted_name_match.format(
            fmt_name=simple_name, fmt_attr=simple_attr
        )
        i_from = from_import_match.format(fmt_name=dotted_name)
        i_from_submod = from_import_submod_match.format(
            fmt_name=simple_name, fmt_attr=simple_attr
        )
...
class FixImports(fixer_base.BaseFix):
    ...
    def fix_submod_import(self, imported, name, node):
        ...
        if missed:
            self.warning(
                node,
                "Imported names not known to crosswind to be part of the package {0}.  Leaving those alone... high probability that this code will be incorrect.".format(
                    name
                ),
            )
            children = [
        ...
        if names:
            for name in names:
                if name.type == token.NAME:
                    ...
                    self.fix_simple_name(name.children[0]) if name.children[
                        0
                    ].type == token.NAME else self.fix_dotted_name(name.children[0])
            ...
            if does_tree_import(
                None, MAPPING[name.value], node
            ) and is_probably_builtin(name):
                self.fix_simple_name(name)
        elif name and attr:
