...
def assignment_source(num_pre, num_post, LISTNAME, ITERNAME):
    ...
    pre = str(num_pre)
    post = str(num_post)
    # This code builds the assignment source from crosswind tree primitives.
...
class FixUnpacking(fixer_base.BaseFix):
    ...
    def transform(self, node, results):
        ...
        a,b,c,d,e,f,g,h,i, = _crosswindlist[:6] + [_crosswindlist[6:-2]] + _crosswindlist[-2:]

        and

        for a,b,*c,d,e in iter_of_iters: do_stuff changes to
        for _crosswinditer in iter_of_iters:
            _crosswindlist = list(_crosswinditer)
            a,b,c,d,e, = _crosswindlist[:2] + [_crosswindlist[2:-2]] + _crosswindlist[-2:]
            do_stuff
        """
        self.LISTNAME = self.new_name("_crosswindlist")
        self.ITERNAME = self.new_name("_crosswinditer")
