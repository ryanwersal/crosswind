...
def get_all_fixer_suites():
    """Return a sorted list of all available fixer suites that can be detected."""
    # FIXME: Use setuptools to discover pypi packages instead of this
    fixer_suites = []
    for name in os.listdir("fixer_suites"):
        if name in ("__pycache__", "__init__.py"):
            continue
        fixer_suites.append("fixer_suites." + name)
    return fixer_suites
...
def get_fixers_from_packages(pkg_names):
    """Return the fully qualified names for fixers in all packages."""
    fixers = []
    for pkg_name in pkg_names:
        fixers.extend(get_fixers_from_package(pkg_name))
    return fixers
