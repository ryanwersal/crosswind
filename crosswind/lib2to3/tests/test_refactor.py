...
_DEFAULT_FIXERS = refactor.get_fixers_from_package(
    "crosswind.lib2to3.tests.data.fixers.myfixes"
)
...
class TestRefactoringTool(unittest.TestCase):
    ...
    def test_fixer_loading_helpers(self):
        ...
        non_prefixed = refactor.get_all_fix_names(
            "crosswind.lib2to3.tests.data.fixers.myfixes"
        )
        prefixed = refactor.get_all_fix_names(
            "crosswind.lib2to3.tests.data.fixers.myfixes", False
        )
        ...
        self.assertEqual(
            full_names,
            [
                "crosswind.lib2to3.tests.data.fixers.myfixes.fix_" + name
...
from __future__ import print_function"""
    ...
    def test_naughty_fixers(self):
        ...
        self.assertRaises(
            refactor.FixerError,
            self.rt,
            fixers=["crosswind.lib2to3.tests.data.fixers.no_fixer_cls"],
        )
        self.assertRaises(
            refactor.FixerError,
            self.rt,
            fixers=["crosswind.lib2to3.tests.data.fixers.bad_order"],
        )
    ...
    def test_crlf_newlines(self):
        ...
        try:
            ...
            fixes = refactor.get_fixers_from_package("crosswind.lib2to3.fixes")
...
"""
    ...
    def test_explicit(self):
        ...
        rt = self.rt(
            fixers=["crosswind.lib2to3.tests.data.fixers.myfixes.fix_explicit"]
        )
        self.assertEqual(len(rt.post_order), 0)

        rt = self.rt(
            explicit=["crosswind.lib2to3.tests.data.fixers.myfixes.fix_explicit"]
        )
